 ...
 * Dateinamen:             Gesamtprogramm_Reaktionsspiel.ino
...
unsigned long Anfangszeit,      // Anfangszeit der Zeitmessung
              ...
              Reaktionszeit,    // errechnete Reaktionszeit
              Zufallszeit;      // zufaellig generiete Zeit (0 <= Wert < 4001)
/* ------------------------------------------------------------------------------ */
...
// Anzeigen zuruecksetzten
void Zuruecksetzung(char Punktzahl_Pin1, char Punktzahl_Pin2, char Reset_Pin);



/* Hauptprogramm ---------------------------------------------------------------- */
void loop() {
  Zuruecksetzung(P_S1, P_S2, R_S);
  /* AP7. ----------------------------------------------------------------------- */
  /* AP3. --------------------------------------------------------------------- */
  Start_Taster(S_S);
 ...
 // Serial.print
  ...
  }while(Pu_Za1 < 9 && Pu_Za2 < 9);   // fuehrt Reaktionstest aus, bis
  ...
  Pu_Za1 = 0;                         // Punktzahl1 zuruecksetzten
  Pu_Za2 = 0;                         // Punktzahl2 zuruecksetzten
  /* ---------------------------------------------------------------------------- */
}
/* ------------------------------------------------------------------------------ */

/* Unterprogrammedefinition ----------------------------------------------------- */
// Start-Signal einlesen
void Start_Taster(char Start_Pin){
...
void Zuruecksetzung(char Punktzahl_Pin1, char Punktzahl_Pin2, char Reset_Pin){
  ...
  for(y = 1;y > -1; y--){
    delay(100);
    digitalWrite(Punktzahl_Pin1, y);
    digitalWrite(Punktzahl_Pin2, y);
  }
